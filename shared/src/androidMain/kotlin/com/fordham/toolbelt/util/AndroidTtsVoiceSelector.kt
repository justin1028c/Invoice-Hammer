...
internal object AndroidTtsVoiceSelector {
    fun applyBestVoice(tts: TextToSpeech, appLocale: AppLocale) {
        val targetLocale = if (appLocale == AppLocale.Spanish) {
            Locale("es", "ES")
        } else {
            Locale.US
        }
        ...
        val best = voices
            .asSequence()
            .filter { voice ->
                val loc = voice.locale ?: return@filter false
                if (appLocale == AppLocale.Spanish) {
                    loc.language == "es"
                } else {
                    loc.language == "en" && (loc.country.isNullOrEmpty() || loc.country.equals("US", ignoreCase = true))
                }
            }
